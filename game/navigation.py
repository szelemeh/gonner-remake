...
class Navigator(metaclass=NavigatorMeta):
    ...
    def show_tutorial_done_screen(self):
        self.__show_screen_with_text_n_finish(
    ...
    def go_to_store(self):
        ...
        self.drawer.draw_text(
            'Things you can buy: (1) Extra speed: 50g    (2) Extra HP: 50g',
            22, WHITE, WIDTH / 2, HEIGHT / 2)
        self.drawer.draw_text('Press Enter to exit store', 22, WHITE,
                              WIDTH / 2, HEIGHT * 5 / 6)
        pg.display.flip()
        waiting = True
        while waiting:
            ...
            for event in pg.event.get():
                if event.type == pg.KEYDOWN and event.key == pg.K_1:
                    if self.game.player.money >= 50:
                        ...
                        self.drawer.draw_text('Not enough money', 20, WHITE,
                                              WIDTH / 2, HEIGHT * 5 / 6 - 100)
                        pg.display.flip()
                ...
                if event.type == pg.KEYDOWN and event.key == pg.K_2:
                    if self.game.player.money >= 50:
                        ...
                        self.game.player.money -= 50
                        self.game.screen.fill(RED, (0, 0, 200, 200))
                        self.drawer.draw_text('Bought hp', 20, WHITE,
                                              WIDTH / 2, HEIGHT * 3 / 4 - 100)
                        self.game.draw_stats_bar()
                        pg.display.flip()
                    else:
                        self.drawer.draw_text('Not enough money', 20, WHITE,
